...
- Модуль чтения листов `AI`, `RTD`, `DO` и `Phase` (токи линий, RTD-точки и фазировка линий).
- Модуль генерации файла `GVL.txt` на основе параметров из `PLC`.
- Модуль генерации файла `Proc_IO.txt` для блока инициализации/обработки RTD, генерации токов линий по данным `AI/DO` и инициализации фазировки.

## Структура
- `st_codegen/xlsx_plc_reader.py` — парсинг XLSX (без внешних библиотек), извлечение параметров PLC, AI/RTD/DO-точек и фазировки.
