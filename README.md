...
- Модуль чтения листов `RTD` (теги, модули, каналы) и `Phase` (фазировка линий).
- Модуль генерации файла `GVL.txt` на основе параметров из `PLC`.
- Модуль генерации файла `Proc_IO.txt` для блока инициализации/обработки RTD и инициализации фазировки.

## Структура
- `st_codegen/xlsx_plc_reader.py` — парсинг XLSX (без внешних библиотек), извлечение параметров PLC, RTD-точек и фазировки.
